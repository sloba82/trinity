...
<div class="flex mx-auto justify-center shadow-lg mt-6 prose prose-indigo prose-lg">
    <form action="{{ route('news.comment.create') }}" method="POST" class="w-full max-w-xl bg-white rounded-lg px-4 pt-2">
        @csrf
    ...
    @foreach ($comments as $comment)
        ...
        <div id="comment_{{$comment->id}}" class="bg-white rounded-lg p-3 flex flex-col md:items-start shadow-lg mb-4">
            ...
            <div>
                <form id="{{$comment->id}}" action="{{ route('reply.create', $comment->id) }}" method="POST" >
                    @csrf
                    <input type="hidden" name="comment_id" value="{{$comment->id}}">
                    <div class="w-full md:w-full px-3 mb-2 mt-2">
                    <input  type="text" name="reply_{{$comment->id}}" placeholder="Reply to comment" class="bg-gray-100 rounded border border-gray-400 leading-normal resize-none w-full h-10 py-2 px-3 font-medium placeholder-gray-700 focus:outline-none focus:bg-white" />
                    </div>
